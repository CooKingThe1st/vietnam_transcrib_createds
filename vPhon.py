...
def trans(word, dialect, glottal, pham, cao, palatals):
    ...
    if dialect == 'v':
        from Rules.north_CMU import onsets, nuclei, codas, tones, onglides, offglides, onoffglides, qu, gi
    if dialect == 'n':
    ...
    if pham or cao:
        ...
        if dialect == 'v': from Rules.north_CMU import tones_p
    ...
    if l > 0:
        ...
        if word[l-2:l] in codas:        # if two-character coda
            ...
            cod = codas[word[l-1]]
            cOffset = 1
#get length of onset and coda, from here could already create phone

        #if word[0:2] == 'gi' and cod and len(word) == 3:  # if you just have 'gi' and a coda...
        if word[0:2] in gi and cod and len(word) == 3:  # if you just have 'gi' and a coda...giò
            nucl = word[2:3]
            ons = word[0:2]
            # Special case
        else:
            nucl = word[oOffset:l-cOffset]
#OUR MAIN INTEREST IS BOUT THE TONE
        # print("HELP")
        # print( ' ONS ', ons, "NUCL ", nucl, ' COD ', cod)
        if nucl in nuclei:
            if oOffset == 0:
                if glottal == 1: # mostly dont use gloattal but palatals
        ...
        elif nucl in onglides and ons != 'qu': # if there is an onglide..., actually dont need that, coz it actually change according to next phoneme
            nuc = onglides[nucl]                # modify the nuc accordingly
            ons = ons  # dont really need to modify
            # if ons:                             # if there is an onset...
            #     if isinstance(ons, list):      # 2 char 
        ...
        if tonelist:
            ...
            if not (pham or cao):
                ...
                ton = '1'
#ONLY USE THE TOKENIZE, DONT USE PHONE, USE GRAPHEME AS IT IS
        ...
        #     # Modification for 8-tone system, DONT CARE
        #     if cao == 1:
...
def convert(word, dialect, glottal, pham, cao, palatals,delimiter = ''):
    ...
    # HAVE CHANGED, if error immedialt assert
    # if parts is None or None in parts:
    #     return ['['+word+']']
    # if any(isinstance(x, list) for x in parts):
    #     for i,p in enumerate(parts):
...
def process_word(word, dialect, glottal, pham, cao, palatals, tokenize, delimiter = ''):
    ...
    ipa = []
    for x in values:
        for phone in convert(x, dialect, glottal, pham, cao, palatals, delimiter):
            ipa.append(phone)
    while (ipa[-1] == ' '):  del ipa[-1]
    return ipa
